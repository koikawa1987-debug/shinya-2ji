...
`docs/portraits/<id>.webp` を置くと、名鑑と人物面にその人の顔写真が出る。置いていない人は何も出ない（欠けても紙面が崩れない）。どれが置いてあるかは `docs/data/portraits.json` に書き出されるので、絵のない人のぶんの 404 は飛ばない。

絵を描かせるためのプロンプトは、人物の外見・肩書・道具・癖から自動で組み立てられる。

```bash
npm run portraits
```

`portraits/prompts.md` に55名ぶんが出る。紙面が単色なので、**新聞に刷られた顔写真**（白黒・網点・インクのにじみ）に寄せた指定になっている。カラーで作ると他の要素と釣り合わない。
